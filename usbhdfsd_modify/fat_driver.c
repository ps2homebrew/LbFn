...
int fat_getDirentry(fat_direntry_sfn* dsfn, fat_direntry_lfn* dlfn, fat_direntry* dir ) {
	...
	int offset;
	int cont;
	static unsigned char uname[260]={0,0,0,0};
	...
	if (dlfn->rshv == 0x0F && dlfn->reserved1 == 0x00 && dlfn->reserved2[0] == 0x00) {
		//long filename - almost whole direntry is unicode string - copy it
		...
		for (i = 0; i < 10 && cont; i+=2) {
			if (dlfn->name1[i]==0 && dlfn->name1[i+1] == 0) {
				uname[offset] = 0; //terminate
				uname[offset+1] = 0;
				cont = 0; //stop
			} else {
				uname[offset++] = dlfn->name1[i];
				uname[offset++] = dlfn->name1[i+1];
				//offset++;
			}
		}
		//name - 2nd part
		for (i = 0; i < 12 && cont; i+=2) {
			if (dlfn->name2[i]==0 && dlfn->name2[i+1] == 0) {
				uname[offset] = 0; //terminate
				uname[offset+1] = 0; //terminate
				cont = 0; //stop
			} else {
				uname[offset++] = dlfn->name2[i];
				uname[offset++] = dlfn->name2[i+1];
				//offset++;
			}
		}
		//name - 3rd part
		for (i = 0; i < 4 && cont; i+=2) {
			if (dlfn->name3[i]==0 && dlfn->name3[i+1] == 0) {
				...
				uname[offset++] = dlfn->name3[i];
				uname[offset++] = dlfn->name3[i+1];
				//offset++;
			}
		}
