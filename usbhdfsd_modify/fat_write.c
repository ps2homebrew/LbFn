...
*/
void setLfnEntry(unsigned char* uname, int nameSize, unsigned char chsum, unsigned char* buffer, int part, int maxPart){
        int i,j;
	...
	nameStart = 13 * part;
        ...
        for (i = 0; i < j; i++) {
        	name[i*2]   = uname[(nameStart + i)*2];
        	name[i*2+1] = uname[(nameStart + i)*2 +1];
        }
...
*/
int createShortNameMask(unsigned char* lname, unsigned char* sname) {	// remaked by nika
	int i,j;
	int k,a;
	int st=-1;
	int ext=-1;
	...
	if (sname[0] == 32) {
		//sname[0] = '_';	// dummy name "_MB_" copy (ex. "_MB_~255")
		//sname[1] = 'M';
		//sname[2] = 'B';
		//sname[3] = '_';
		strncpy(&sname[0], "_MBCS_PS", 8);
	}
	return k;
}
