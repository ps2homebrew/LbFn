 ...
 * 2003/05/12 | optimized DCT ( 20-bits fixed point, etc...) -> line 407-464 ( by nikq, I.Tak. )
 * 2003/09/27 | PICTURE0.BIN(DLL)用に改造 ( by くーみん )
 * 2003/09/28 | 各種バグフィクス＆多少の最適化 ( by H.Kawai )
 * 2004/08/25 | 誤差拡散対応 (by I.Tak.)
 * 2004/09/08 | partial output (still decoding all mcu block... by I.Tak.)
 * 2005/03/27 | warningの削除(unused variable)、下記の変更。byあっきぃ
 *              'int jpeg_decode_mcu()' -> 'void jpeg_decode_mcu()'
 */
...
int jpeg_decode(JPEG *jpeg, UCHAR *rgb, int b_type, int xsz, int ysz, int x0, int y0);
...
//		return 8;
	...
	return jpeg_decode(jpeg, buf, b_type, jpeg->width, jpeg->height, 0, 0);
}

int decode0_JPEGpart(struct DLL_STRPICENV *env, int xsz, int ysz, int x0, int y0,int size, UCHAR *fp0, int b_type, UCHAR *buf, int skip)
{
	JPEG *jpeg = (JPEG *) (((int *) env) + 128);
	jpeg->fp = fp0;
	jpeg->fp1 = fp0 + size;
	...
	jpeg->width_buf = skip / (b_type & 0x7f) + xsz;
	return jpeg_decode(jpeg, buf, b_type, xsz,ysz, x0,y0);
}
// -------------------------- I/O ----------------------------
...
/* 中間バッファのポインタ (固定)
 * →バッファのポインタ (固定)
 * ↓バッファのポインタ (毎回変わる, 書いた分だけ進む)
 */
// YCrCb=>RGB

int jpeg_decode_yuv(JPEG *jpeg, int h, int v, unsigned char *rgb, int b_type,
		    int*utod,int*ltor,int*buf,int xo,int yo,int xe,int ye)
{
	int x0, y0, x, y, x1, y1;
	int *py, w;
	const int mw = jpeg->mcu_width;
	const int mh = jpeg->mcu_height;
	...
	x1 += x0;
	y1 += y0;
	if (x1 <= xo || xe <= x0 || y1 <= yo || ye <= y0)
	  return 0;		/* whole block clipped */
	...
	if (ye < y1)
	  y1 = ye;
	rgb += ((y - yo) * jpeg->width_buf + (x - xo)) * (b_type & 0x7f);
	py = jpeg->mcu_buf + (y - y0) * mw + x - x0;
	x1 -= x;		/* transfer length */
	y1 -= y;
	...
	//	rgb += x1 * 2;
	    ...
	    if (b_type == 0x0002) {
	      ...
	      rgb += x1 * 2;
	    } else if (b_type == 0x0801) {
	      gosakaku6(ltor[y], utod, x1, buf, rgb);
	      rgb += x1;
	    } else if (b_type  == 0x0201) {
	      gosakaku4(ltor[y], utod, x1, buf, rgb);
	      rgb += x1;
	    } else if ((b_type & 0xff) == 0x01) {
	      gosakaku3(ltor[y], utod, x1, buf, rgb);
	      rgb += x1;
	    }
...
//	if ((b_type & 0xff) != 4 &&
//	    sizeof(struct DLL_STRPICENV) - 512 - sizeof(JPEG)
//	    - jpeg->mcu_height - jpeg->mcu_width < jpeg->width)
//	  return 1;		/* 誤差拡散バッファが足りない */
	...
	// 1ブロック展開するかもしれない
	mcu_count = 0;
	for (h = 0; h < jpeg->width; h++)
	  (((int*)(jpeg+1))+jpeg->mcu_height+jpeg->mcu_width)[h] = 0;
	for (v = 0; v < v_unit; v++) {
	  int *ltor = (int*)(jpeg+1);
	  int *buf = ltor + jpeg->mcu_height;
	  int *utod = buf + jpeg->mcu_width;
	  for (h = 0; h < jpeg->mcu_height; h++)
	    ltor[h] = 0;
	  for (h = 0; h < h_unit; h++) {
	    mcu_count++;
	    jpeg_decode_mcu(jpeg, zigzag_table);
	    utod += jpeg_decode_yuv(jpeg, h, v, rgb, b_type & 0x7fff,
				    utod, ltor, buf, x0,y0,x0+xsz,y0+ysz);
 ...
 * mh = jpeg->mcu_height;
 * w  = jpeg->width;
 * ltor = delta_buffer: 誤差バッファ→
 * buf  = delta_buffer+mh: 中間バッファ
 * utod = delta_buffer+mh+mw: 誤差バッファ↓
 * delta_buffer+mh+mw+w: バッファ終わり */
